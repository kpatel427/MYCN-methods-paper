...
MYDIRWITHWIGS='/mnt/isilon/maris_lab/target_nbl_ngs/KP/enhancer-rank-list-H3K27Ac/LILY/data/'
...
for(x in cl){
    ...
    files.regions=files[which(  (sapply(as.character(files),FUN=function(x) {strsplit(x,split="_",fixed = F)[[1]][2]})=="peaks.narrowPeak" | sapply(as.character(files),FUN=function(x) {strsplit(x,split="_",fixed = F)[[1]][3]})=="peaks.narrowPeak")
                                & (sapply(as.character(files),FUN=function(x) {strsplit(x,split="_",fixed = T)[[1]][1]})==type)  )]
    ...
    files.Wig=paste0(samplesToProcess,".bw")
